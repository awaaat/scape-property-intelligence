  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setPricing(p);
        if (isLoggedIn()) {
          try { const u = await fetchMyUsage(); if (!cancelled) setUsage(u); } catch {}
        }
      } catch { if (!cancelled) setError("Couldn't load pricing right now. Please refresh."); }
        ...
        <div className="pc-container">
          {loading && (
            <div style={{ display: "flex", justifyContent: "center", padding: 60 }}>
              <Loader2 size={26} style={{ animation: "spin 0.8s linear infinite", color: "var(--lime)" }} />
            </div>
          )}
          {!loading && error && (
            <div style={{ display: "flex", gap: 10, alignItems: "center", padding: "16px 20px", background: "rgba(166,64,46,0.1)", border: "1px solid var(--red)", color: "var(--red)", maxWidth: 560, margin: "0 auto" }}>
